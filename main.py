...
from werkzeug.security import generate_password_hash, check_password_hash, pbkdf2_bin
...
def login_with_value(user_id):
    email = db.session.query(User).filter_by(id=user_id).first().email
...
def login():
    if request.method == "POST":
        ...
        if user == None:
            ...
            flash("Your password is inccorect!")
